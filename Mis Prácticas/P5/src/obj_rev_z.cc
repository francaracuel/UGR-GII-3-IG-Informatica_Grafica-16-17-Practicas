...
//
ObjRevZ::ObjRevZ(bool ask) : ObjRev(ask){
...
ObjRevZ::ObjRevZ(bool ask, unsigned int sides) : ObjRev(ask, sides){
...
// Crea la geometría del objeto por revolución con un número de caras dadas
// y un fichero
ObjRevZ::ObjRevZ(unsigned int sides, string f) : ObjRev(sides, f){
...
ObjRevZ::ObjRevZ(string f, unsigned int sides, bool top, bool bottom) : 
...
void ObjRevZ::createGeometry(){
    ...
    mesh = new Mesh(listVertex.size()/3, listIndex.size(), listVertex, listIndex, 30, 182, 129, 182, 30, 83, axisSize, false, side, listVertexOutline.size(), listVertexOutline);
...
void ObjRevZ::setPoints(){
    
    // coordenada1.Z > coordenada2.Z > coordenada3.Z > ... > coordenadaN.Z
    // Si esto no se cumple se le da la vuelta a las coordenadas
    if(listVertexOutline[2] < listVertexOutline[5]){
    ...
    if(topCover){
        ...
        // de la tapa como si no, el punto central valdrá z=0. x valdrá la
        // altura que se reciba. y valdrá la profundidad que se reciba.
        ...
        get<0>(top) = 0;
        get<1>(top) = 0;
        get<2>(top) = listVertexOutline[2];
                 
    }
    
    // Se comprueba si el primer punto está en el eje Z
    if(listVertexOutline[0] == 0 && listVertexOutline[1] == 0){
    ...
    if(bottomCover){
        ...
        get<0>(bottom) = 0;
        get<1>(bottom) = 0;
        get<2>(bottom) = listVertexOutline[listVertexOutline.size()-1];
                        
    }
    
    // Se comprueba si el primer punto está en el eje Y
    if(listVertexOutline[listVertexOutline.size()-3] == 0 && listVertexOutline[listVertexOutline.size()-2] == 0){

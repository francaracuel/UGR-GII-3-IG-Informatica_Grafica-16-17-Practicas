...
//
ObjRevY::ObjRevY(bool ask) : ObjRev(ask){
...
ObjRevY::ObjRevY(bool ask, unsigned int sides) : ObjRev(ask, sides){
...
// Crea la geometría del objeto por revolución con un número de caras dadas
// y un fichero
ObjRevY::ObjRevY(unsigned int sides, string f) : ObjRev(sides, f){
...
ObjRevY::ObjRevY(string f, unsigned int sides, bool top, bool bottom) : 
...
void ObjRevY::setPoints(){
    
    // coordenada1.X > coordenada2.X > coordenada3.X > ... > coordenadaN.X
    // Si esto no se cumple se le da la vuelta a las coordenadas
    if(listVertexOutline[0] < listVertexOutline[3]){
    ...
    if(topCover){
        ...
        // de la tapa como si no, el punto central valdrá y=0. x valdrá la
        ...
        get<0>(top) = listVertexOutline[0];
        get<1>(top) = 0;
        get<2>(top) = 0;
                 
    }
    
    // Se comprueba si el primer punto está en el eje X
    if(listVertexOutline[1] == 0){
    ...
    if(bottomCover){
        ...
        get<0>(bottom) = listVertexOutline[listVertexOutline.size()-3];
        get<1>(bottom) = 0;
        get<2>(bottom) = 0;
    ...
    // Se comprueba si el primer punto está en el eje X
    if(listVertexOutline[listVertexOutline.size()-2] == 0){
